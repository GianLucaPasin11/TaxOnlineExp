...
class Decide(Page):
    ...
    def js_vars(player: Player):
        return dict(rows=Constants.rows[player.round_number])
...
class Results(Page):
    ...
    def vars_for_template(player: Player):
        ...
        for p in player.in_all_rounds():
            row = Constants.rows[p.round_number][p.choice]
            to_self_total += row['to_self']
            to_other_total += row['to_other']

        avg_self = to_self_total / Constants.num_rounds
        avg_other = to_other_total / Constants.num_rounds
